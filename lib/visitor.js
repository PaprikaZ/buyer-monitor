...
var AmazonCNVisitor, AmazonJPVisitor, AmazonUSVisitor, JingDongVisitor, MANDATORY_BASE_FIELDS, MANDATORY_EXPAND_FIELDS, Visitor, config, createParser, db, messenger, request, s, util,
...
Visitor = (function() {
  ...
  Visitor.prototype.processPage = function(html) {
    var date, delayDebugMsg, pushMsg, result;
    date = new Date();
    ...
    if (this.seed.verdict(result)) {
      ...
      this.client.lpush(config.redisDelayQueueKey, JSON.stringify(pushMsg));
    }
  ...
  Visitor.prototype.parsePage = function(html) {
    var attr, mountField, result, self, val, _ref;
    self = this;
    result = {};
