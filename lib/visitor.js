...
Visitor = (function() {
  ...
  Visitor.prototype.processPage = function(html) {
    ...
    if (this.seed.verdict(result)) {
      ...
      this.client.lpush(config.redisDelayQueueKey, JSON.stringify(delayMsg), function(err) {
        return err && db.redisErrorRethrow(err);
      });
      this.client.lpush(config.redisPushQueueKey, JSON.stringify(result), function(err) {
        return err && db.redisErrorRethrow(err);
      });
    }
    result.date = date.toUTCString();
    this.client.lpush(config.redisHistoryKey, JSON.stringify(result), function(err) {
      return err && db.redisErrorRethrow(err);
    });
  };
