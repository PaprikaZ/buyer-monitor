...
listHandler = function() {
  ...
  if (0 < products.length) {
    products.forEach(function(elt, index, arr) {
      ...
      if (elt.benefits) {
        output += util.format('benefits match \/%s\/%s', elt.benefits.regex, elt.benefits.option);
      }
      console.log(output);
    });
  } else {
...
addHandler = function(argv) {
  ...
  analyze = function() {
    ...
    iter(argv);
    if (MANDATORY_BASE_FIELDS.reduce((function(partial, field) {
      return partial && record[field];
    }), true) && MANDATORY_VERDICT_FIELDS.reduce((function(partial, field) {
      return partial || record[field];
    }), false)) {
      return record;
    } else {
