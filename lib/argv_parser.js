...
var AVAILABLE_COMPARES, MANDATORY_BASE_FIELDS, MANDATORY_VERDICT_FIELDS, addHandler, config, fs, helpHandler, lackArgHandler, listHandler, path, productFile, removeHandler, resetHandler, seed, unknownArgvHandler, util;
      ...
      MANDATORY_VERDICT_FIELDS.map(function(field) {
        if (product[field]) {
          if (product[field].compare === 'equal') {
            output += util.format(', %s? %s', field, product[field].target);
          } else if (field === 'benefits' && product[field].compare === 'match') {
            output += util.format(', benefits match \/%s\/%s', product[field].regex, product[field].option);
          } else {
            output += util.format(', %s %s %s', field, product[field].compare, product[field].target);
          }
        }
      });
...
addHandler = function(argv) {
  ...
  analyze = function() {
    ...
    if (MANDATORY_BASE_FIELDS.every(function(field) {
      return record[field];
    }) && MANDATORY_VERDICT_FIELDS.some(function(field) {
      return record[field];
    })) {
      return record;
    } else {
